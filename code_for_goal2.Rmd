...
author: "Akshay Kumar Dhar and Vinu vanathayan (Team-12)"
date: "2023-04-24"
output:
...
data1 <- read.csv("C:\\Users\\AKSHAY KUMAR DHAR\\Desktop\\datafiles\\greypartridge_features.csv")
data2 <- read.csv("C:\\Users\\AKSHAY KUMAR DHAR\\Desktop\\datafiles\\stockdove_features.csv")
data3 <- read.csv("C:\\Users\\AKSHAY KUMAR DHAR\\Desktop\\datafiles\\turtledove_features.csv")
data4 <- read.csv("C:\\Users\\AKSHAY KUMAR DHAR\\Desktop\\datafiles\\yellowhammer_features.csv")
...
set.seed(123)
greypartridge <- read.csv("C:\\Users\\AKSHAY KUMAR DHAR\\Desktop\\datafiles\\greypartridge_features.csv")
stockdove <- read.csv("C:\\Users\\AKSHAY KUMAR DHAR\\Desktop\\datafiles\\stockdove_features.csv")
turtledove <- read.csv("C:\\Users\\AKSHAY KUMAR DHAR\\Desktop\\datafiles\\turtledove_features.csv")
yellowhammer <- read.csv("C:\\Users\\AKSHAY KUMAR DHAR\\Desktop\\datafiles\\yellowhammer_features.csv")
...
ggplot(OUTLIERS_data, aes(x = "", y = outliers, fill = Bird_Species)) +
  ...
  coord_polar(theta = "y") +
  theme_void() +
  labs(title = "Outliers Counts", fill = "Bird species") +
...
# The within-cluster sum of squares (WSS) is a measure of the compactness of clusters in a clustering algorithm. Lower WSS values do indeed indicate that the data points within each cluster are closer to each other, which implies better clustering
#To determine which species sound clips clump together more well than others, We compare the WSS values for each species
...
ggplot(WSS_SCORE, aes(x = Bird_Species, y = WSS_Score, fill = Bird_Species)) +  
  ...
  geom_text(aes(label = round(WSS_Score,0), y = round(WSS_Score,0)), position = position_dodge(width = 0), vjust = -0.2) +
